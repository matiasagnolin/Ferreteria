...
<body ng-app="app">
  ...
  <div class="container grid">
		...
		<c:forEach items="${venta.detalleventa}" var="item">
		<div class="row">
			<div class="col-sm-2" id="Id">${item.producto.getDescripcion_Producto()}</div>
			<div class="col-sm-2">${item.producto.getPrecio_Unitario_Producto()}</div>
			<div class="col-sm-2">${item.cantidad}</div>
			<div class="col-sm-2">${item.totalParcial}</div>
			</div>			
			</c:forEach>
		</div>
